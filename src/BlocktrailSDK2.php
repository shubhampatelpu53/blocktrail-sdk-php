 ...
 * Class BlocktrailSDK
 */
class BlocktrailSDK2 implements BlocktrailSDKInterface {
    /**
    ...
    public function __construct($apiKey, $apiSecret, $network = 'BTC', $testnet = false, $apiVersion = 'v1', $apiEndpoint = null) {
        ...
        if (is_null($apiEndpoint)) {
            $apiEndpoint = getenv('BLOCKTRAIL_SDK_API_ENDPOINT') ?: "https://chain.api.btc.com/v3/";
//            $apiEndpoint = "{$apiEndpoint}/{$apiVersion}/{$apiNetwork}/";
    ...
    public function block($block) {
        ...
        $data = self::handleBtccomApiResponse($response->body());

        return self::convertBlock($data);
    }

    public static function handleBtccomApiResponse($responseBody) {
        $response = self::jsonDecode($responseBody, true);

        if (isset($response['err_no']) && $response['err_no'] !== 0) {
            if ($response['err_no'] === 1) {
                throw new ObjectNotFound($response['err_msg']);
            } else if ($response['err_no'] === 2) {
                throw new EndpointSpecificError($response['err_msg']);
            } else {
                throw new UnknownEndpointSpecificError($response['err_msg']);
            }
        }
    ...
    public static function convertBlock($data) {
        $data['version'] = (string)$data['version'];
        $data['merkleroot'] = (string)$data['mrkl_root'];
        $data['difficulty'] = (int)$data['difficulty'];

        $data['block_time'] = (new \DateTime("@{$data['timestamp']}"))->format(\DateTime::ISO8601);
        $data['arrival_time'] = (new \DateTime("@{$data['timestamp']}"))->format(\DateTime::ISO8601);

        $data['prev_block'] = $data['prev_block_hash'];
        $data['next_block'] = $data['next_block_hash'];
        $data['byte_size'] = $data['size'];
        $data['transactions'] = $data['tx_count'];

        // @TODO: remove
        $data['miningpool_url'] = null;
        $data['miningpool_name'] = null;
        $data['miningpool_slug'] = null;

        return $data;
    }

    public static function convertBlockTx($row) {
        $row['time'] = (new \DateTime("@{$row['block_time']}"))->format(\DateTime::ISO8601);

        // @TODO: remove
        $row['estimated_value'] = $row['outputs_value'];
        $row['estimated_change'] = null;
        $row['estimated_change_address'] = null;

        $row['total_output_value'] = $row['outputs_value'];
        $row['total_fee'] = $row['fee'];
        if ($row['is_coinbase']) {
            // @TODO: ??
            $row['total_input_value'] = $row['total_output_value'] - $row['fee'];
        } else {
            $row['total_input_value'] = $row['total_output_value'] + $row['fee'];
        }

        $row['opt_in_rbf'] = false;

        foreach ($row['inputs'] as $idx => &$input) {
            $input['index'] = $idx;
            $input['address'] = isset($input['addresses']) && count($input['addresses']) ? $input['addresses'][0] : null;

            $input['multisig'] = null;
            $input['multisig_addresses'] = null;

            $input['value'] = $input['prev_value'];
            $input['script_signature'] = $input['script_hex'];
            $input['output_hash'] = $input['prev_tx_hash'];
            $input['output_index'] = $input['prev_position'];
            if ($input['output_index'] === -1) {
                $input['output_index'] = 0;
            }

            if ($input['sequence'] < TransactionInput::SEQUENCE_FINAL) {
                $row['opt_in_rbf'] = true;
            }

            if ($row['is_coinbase']) {
                $input['value'] = $row['total_input_value'];
                $input['output_hash'] = null;
                $input['type'] = 'coinbase';
            } else {
                $input['type'] = self::convertTxType($input['type']);
            }
        }

        foreach ($row['outputs'] as $idx => &$output) {
            $output['index'] = $idx;
            $output['address'] = isset($output['addresses']) && count($output['addresses']) ? $output['addresses'][0] : null;

            $output['multisig'] = null;
            $output['multisig_addresses'] = null;

            $output['script'] = $output['script_asm'];
            $output['spent_hash'] = $output['spent_by_tx'];
            $output['spent_index'] = $output['spent_by_tx_position'];
            if ($output['spent_index'] === -1) {
                $output['spent_index'] = 0;
            }

            $output['type'] = self::convertTxType($output['type']);
        }

        return $row;
    }

    public static function convertTxType($type) {
        // @TODO: more types
        switch ($type) {
            case 'P2PKH_PUBKEY':
                return 'pubkey';
    ...
    public function blockTransactions($block, $page = 1, $limit = 20, $sortDir = 'asc') {
        if ($sortDir !== 'asc') {
            throw new \InvalidArgumentException("sortdir is deprecated");
        }
        ...
        $response = $this->client->get("block/{$block}/tx", $queryString);
        $data = self::handleBtccomApiResponse($response->body());
        ...
        foreach ($data['data'] as $idx => $row) {
            $data['data'][$idx] = self::convertBlockTx($row);
        }
